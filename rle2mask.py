...
def files_mask2rle_single(imgx):
    '''
    批量将mask转为rlu
    :param path:
    :return:
    '''
    csv = open(r'predict.csv', 'w')
    img = Image.open(imgx).convert('L')
    img = np.array(img)
    w, h = img.shape[1::-1]
    # img = img[:, :, 0]
    img = img // 255
    result = mask2rle(img)
    csv.writelines("{},{} {},{}\n".format(imgx, w, h, result))


def files_mask2rle_single_from_np(file_path,file_name,imgx):
    '''
    批量将mask转为rlu
    :param file_path: 路径
    :param imgx:
...
if __name__ == '__main__':
    files_mask2rle_single(r'D:\testpng\0cdf5b5d0ce1_01_mask.png')
